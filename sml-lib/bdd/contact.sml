    ...
    in
      ...
      then
          ...
          in
              ...
              (case (mbe_manifold, mbe_old_manifold) of
                   (NONE, _) => touching := false
                 | (SOME manifold, NONE) => touching := true
                 | (SOME manifold, SOME old_manifold) =>
                   (touching := true;
                    (* Match old contact ids to new contact ids and copy the
                       stored impulses to warm start the solver. *)
                    for 0 (Array.length (#points manifold) - 1)
                        (fn i =>
